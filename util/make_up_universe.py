...
def get_universe():
    ...
    # 사용할 column들 설정
    cols = ['거래량', '전일거래량', '등락률', '영업이익증가율']

    df['등락률'] = df['등락률'].str.rstrip('%').astype(float)
    # column들을 숫자타입으로 변환(Naver Finance를 크롤링해온 데이터는 str 형태)
    df[cols] = df[cols].astype(float)

    # 유니버스 구성 조건 (1)~(4)를 만족하는 데이터 가져오기
    df = df[(df['거래량'] > 0) & (df['영업이익증가율'] > 0) & (df['등락률'] > 0) & (df['거래량'] - df['전일거래량'] > 0) & (~df.종목명.str.contains("지주")) & (~df.종목명.str.contains("홀딩스"))]


    # RANK_VALUE을 기준으로 정렬
    #df = df.sort_values(by=['영업이익증가율'])
